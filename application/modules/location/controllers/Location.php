...
class Location extends MY_Controller
{
	...
	{				   					   
		...
		{  
			...
			$data[]= array(
				...
				'<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('location/country/edit/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a>
            	 <a title="Delete" class="delete btn btn-sm btn-danger" href="'.base_url('location/country/del/'.$row['id']).'" onclick="return confirm(\'Do you want to delete ?\')" > <i class="fa fa-trash-o"></i></a>'
			);
		}
		$records['data']=$data;
	...
	public function country_add()
	{
		if($this->input->post()){
			...
			if ($this->form_validation->run() === FALSE) {
				$this->load->view('template/_header', $data);
				$this->load->view('location/country_add', $data);
				$this->load->view('template/_footer', $data);
				return;
			}
	...
	public function country_edit($id=0)
	{
		...
		if($this->input->post()){
			...
			if ($this->form_validation->run() === FALSE) {
				$this->load->view('template/_header', $data);
				$this->load->view('location/country_edit', $data);
				$this->load->view('template/_footer', $data);
				return;
			}
	...
	{				   					   
		...
		{  
			...
			$data[]= array(
				...
				$row['name'],
				'<span class="btn btn-xs btn-success" title="status">'.$status.'<span>',				
				'<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('location/state/edit/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a>
            	 <a title="Delete" class="delete btn btn-sm btn-danger" href="'.base_url('location/state/del/'.$row['id']).'" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>'
			);
		}
		$records['data']=$data;
		echo json_encode($records);						   
	...
	public function state_add()
	{
		if($this->input->post()){
			...
			redirect(base_url('location/state'));
		}
		else{
	...
	public function state_edit($id=0)
	{
		...
		if($this->input->post()){
			...
			if ($this->form_validation->run() === FALSE) {
				$this->load->view('template/_header', $data);
				$this->load->view('location/state_edit', $data);
				$this->load->view('template/_footer', $data);
				return;
			}
	...
	{				   					   
		...
		{  
			...
			$data[]= array(
				...
				$row['name'],
				'<span class="btn btn-xs btn-success" title="status">'.$status.'<span>',				
				'<a title="Edit" class="update btn btn-sm btn-warning" href="'.base_url('location/city/edit/'.$row['id']).'"> <i class="fa fa-pencil-square-o"></i></a>
				<a title="Delete" class="delete btn btn-sm btn-danger" href="'.base_url('location/city/del/'.$row['id']).'" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>'
			);
		}
		$records['data']=$data;
		echo json_encode($records);						   

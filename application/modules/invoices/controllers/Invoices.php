<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Invoices extends MY_Controller {
		...
		public function index(){
        	...
        	$this->load->view('invoices/invoice_list', $data);
		...
		public function add()
		{
			...
			if($this->input->post('submit')){
				...
				$this->load->view('template/_header');
        		$this->load->view('invoices/invoice_add', $data);
        		$this->load->view('template/_footer');
			}
			...
			if($this->input->post('submit')){
				...
				if($company_id){
					...
					if($result){
						...
						redirect(base_url('invoices/edit/'.$id));
					}
				}	
			}
			else{
				...
				$this->load->view('template/_header');
        		$this->load->view('invoices/invoice_edit', $data);
        		$this->load->view('template/_footer');
			}
		}
		...
		public function invoice_pdf_download($id=0){
			...
			$data['invoice_detail'] = $this->invoice_model->get_invoice_by_id($id);
			$this->load->view('invoices/invoice_pdf_download', $data);
		}
